...
> 🚀 **Status Update:** v0.7.2 now available with complete Time Tracking support (including date range filtering) and Document Management features.
...
| • View time entries for tasks<br>• Start/stop time tracking on tasks<br>• Add manual time entries<br>• Delete time entries<br>• View currently running timer<br>• Track billable and non-billable time<br>• Filter time entries by date range                                 | • Navigate spaces, folders, and lists<br>• Create and manage folders<br>• Organize lists within spaces<br>• Create lists in folders<br>• View workspace hierarchy<br>• Efficient path navigation                                             |
...
| [get_workspace_time_entries](docs/api-reference.md#time-tracking)     | Get time entries with filtering | Optional filters: `startDate`, `endDate`, task/list/folder/space IDs                                                     |
