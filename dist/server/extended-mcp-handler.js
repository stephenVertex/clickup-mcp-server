...
export class ExtendedMCPHandler {
    ...
    // List CRUD methods
    async createList(spaceId, name, content) {
        const body = { name };
        if (content)
    ...
    // Task CRUD methods
    async createTask(listId, name, description, status) {
        const body = { name };
        if (description)
    ...
    async updateTask(taskId, name, description, status) {
        const body = {};
        if (name)
            body.name = name;
        if (description)
...
//# sourceMappingURL=extended-mcp-handler.js.map